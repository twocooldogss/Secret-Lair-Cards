/**
 * 🔮 SecretLairCards.com – fetchDrops.js v4 (渐进式升级版)
 * ---------------------------------------------------
 * 基于我们现有v3版本，融合ChatGPT v4的核心改进：
...
async function fetchDropData(drop) {
  try {
    // 🧱 数据保护：跳过已有完整数据的drops
    if (drop.image && drop.cards?.length >= 3 && !drop.image.includes('placeholder')) {
...
async function main() {
  console.log("🚀 开始从 Scryfall 获取 Secret Lair 数据 (v4增强版)...");
  ...
  const summary = {
    ...
    skipped: updated.filter((d) => d.image && d.cards?.length >= 3 && !d.image.includes('placeholder')).length
  };
