 ...
 * ---------------------------------------------------
 * 功能：
 *  1. 读取 /data/drops.json
 *  2. 调用 Scryfall API 获取 Drop 的卡牌与封面
 *  3. 多集合搜索 (sld|slu|slc|slp) + 名称映射表
 *  4. 支持三层搜索：精确匹配 → Artist → 模糊匹配
 *  5. 适度数据保护 + 封面去重逻辑
 *  6. 写回 /data/drops.json
 *
 * 运行方式：
 *   node scripts/fetchDrops.js
 *
 * 依赖：
